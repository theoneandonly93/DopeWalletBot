...
export default function TokenList({ tokens = [], chain = 'solana' }) {
  ...
  {tokens.map((t) => {
        ...
        if (!imgSrc) {
          // Chain-specific fallback images if available in /public
          imgSrc = chain === 'bsc' ? '/bnb.svg' : '/sol.svg';
        }
        ...
        return (
          <Link
            ...
            className="flex items-center justify-between py-2 border-b border-[#222] last:border-0 hover:bg-[#0b0b0b] px-2 rounded-md"
          >
            <div className="flex items-center space-x-3">
              <div className="relative">
                <img src={imgSrc} alt={displaySymbol} className="w-8 h-8 rounded-full" />
                <div className="absolute -bottom-1 -right-1 bg-[#0b0b0b] border border-[#222] rounded-full w-5 h-5 flex items-center justify-center">
                  <span className="text-[10px] font-semibold text-white uppercase">{chain === 'bsc' ? 'B' : 'S'}</span>
                </div>
              </div>
              <div>
                <p className="text-sm font-semibold">{displaySymbol}</p>
                <p className="text-xs text-textDim truncate w-40">{t.name || t.tokenName || ''}</p>
                <p className="text-xs text-textDim">{volumeNum > 0 ? `$${volumeNum.toLocaleString()} Vol` : ''}</p>
              </div>
            </div>
